function plotAll(Z_NONE, Z_GAPS, Z_PADS, f_vec, plotstr)

if nargin == 4, plotstr = '.'; end

% rmi = @(z) abs(real(z)) + abs(imag(z));

% l_vec = [10, 20, 40, 80, 160, 320, 640] * 1e-6;

X_PLOT = ((0.5*abs(Z_GAPS(1:end-1,:,:)) + 0.5*abs(Z_GAPS(2:end,:,:))) ./ abs(Z_NONE) - 1) * 100;
Y_PLOT = (abs(Z_PADS) ./ abs(Z_NONE) - 1) * 100;

plotAllXY(X_PLOT, Y_PLOT, f_vec, plotstr)

% % X_PLOT = ((0.5*rmi(Z_GAPS(1:end-1,:,:)) + 0.5*rmi(Z_GAPS(2:end,:,:))) ./ rmi(Z_NONE) - 1) * 100;
% % Y_PLOT = (rmi(Z_PADS) ./ rmi(Z_NONE) - 1) * 100;
% 
